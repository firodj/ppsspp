...
void MIPSDisAsm(MIPSOpcode op, u32 pc, char *out, bool tabsToSpaces, std::string *logs) {
	if (op == 0) {
		sprintf(out,"nop");
	} else {
		disPC = pc;
	...
	while (coreState == CORE_RUNNING)
	{
		...
		while (curMips->downcount >= 0 && coreState == CORE_RUNNING)
		{
			// int cycles = 0;
			{
				...
				if (bbTrace) bbTrace->RecordPC(curMips->pc);
		...
		//2: check for breakpoint (VERY SLOW)
				...
				if (curMips->inDelaySlot)
				{
					...
					if (wasInDelaySlot)
					{
						...
						if (bbTrace) { bbTrace->next_adj_bb(curMips->pc); }
						continue;
					}
					goto again;
				}
